...
def append_songs(ctx, playlist, playnext=False, songs=[]):  # appends songs to the playlist
    ...
    playlist[server][0] += playlist[server][1][0:1000 - length]
...
def create_embed(ctx, playlist, page_num):  # todo add timestamp
    ...
    embed.set_footer(
        text=f'Showing song(s) in the playlist queue from page {page_num+1}/{playlist_length} out of {len(playlist[server][0])} song(s) in the queue')  # bigggggg
    return embed
...
def create_info_embed(self, ctx, number=0, song=None):
    ...
    icon = YouTube.fetch_channel_icon(
        youtube=self.youtube, channelId=song.channelId)
...
async def fetch_songs(self, ctx, url, no_playlists=False):
    if not validators.url(url):  # if url is invalid (implying for a search)
        ...
        song = await YouTube.fetch_from_search(self.youtube, query=url)
