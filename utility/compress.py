...
async def Video(video_full_path, size_upper_bound, two_pass=True, filename_suffix='t'):
    """
    ...
    try:
        ...
        if two_pass:
            ...
            ffmpeg.output(i, output_file_name,
                          **{'c:v': 'libx264', 'b:v': video_bitrate, 'c:a': 'aac', 'b:a': audio_bitrate, 'loglevel': 'quiet'}
                          ).overwrite_output().run()
        ...
        if os.path.getsize(output_file_name) <= size_upper_bound * 1024:
            ...
            return Video(output_file_name, size_upper_bound)
        else:
            return False
