...
variables:
`color`
the color the embeds should be in
...
variables:
`json`
The json response of the search
`song_results`
All the parsed results as a list of new SongResult classes
`best_song_result`
The first index of `song_results`
...
Video's description
`channelId`
Video's Uploader's channel's ID
`channelTitle`
Video's Uploader's channel's Title
`videoId`
ID of the video
`thumbnail`
...
Event loop
`youtube`
YouTube api libary's instance
`channel_icons`
Contains the already fetched channel icons for memoization
`client`
Httpx AsyncClient for requests
...
`similarity`
How similar the result image is to the actual image
`title`
Title of the result image
`image`
Url to the image
...
list index
`embeds`
all the embeds for the results
`results`
all the results
`loop`
Running event loop
