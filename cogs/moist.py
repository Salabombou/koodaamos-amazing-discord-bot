...
async def CreateSpeech(text, token):
    payload = {
        "speech": text + '.~',
        "voice": "cr1tikal",
        "pace": 1
    ...
    async with httpx.AsyncClient(timeout=30) as requests:
        r = await requests.post(data=json.dumps(payload), headers={"Content-Type": "application/json", "Authorization": f"Bearer {token}" }, url='https://api.uberduck.ai/speak')
        r.raise_for_status()
        uuid = r.json()['uuid']
        url = f'https://uberduck-audio-outputs.s3-us-west-2.amazonaws.com/{uuid}/audio.wav'
        speech = await requests.head(url)
...
class moist(commands.Cog):
    ...
    async def moist(self, ctx, *, arg="Hi there!"):
        ...
        async with ctx.typing():
            wav = await CreateSpeech(text=arg, token=self.token)
